...
export async function POST(request: Request) {
  ...
  const workspace: WorkspaceData = { company: { name: companyName, departments: [], defaultRole: "employee", contactName: user.displayName, contactEmail: normalizeEmail(user.email), contactPhone: "" }, jobs: [], crew: [], mileage: [] }; const saved = await saveWorkspace(normalizeEmail(user.email), workspace); if (!saved.ok) return saved.response;
...
export async function PUT(request: Request) {
  ...
  if (input.version && current.updatedAt !== input.version) return Response.json({ error: "Workspace changed on another device", updatedAt: current.updatedAt }, { status: 409 });
  ...
  const saved = await saveWorkspace(access.ownerEmail, workspace); if (!saved.ok) return saved.response; const memberConflicts = canManageTeam ? await syncMemberships(access.ownerEmail, workspace.crew) : [];
...
export async function PATCH(request: Request) {
  ...
  else return Response.json({ error: "Invalid action" }, { status: 400 }); const saved = await saveWorkspace(access.ownerEmail, workspace); if (!saved.ok) return saved.response; return Response.json({ saved: true, updatedAt: saved.updatedAt });
}
export async function DELETE() {
  ...
  if (row && access.crewId) { const workspace = parseWorkspace(row.data); workspace.crew = workspace.crew.filter((m) => m.id !== access.crewId); workspace.jobs = workspace.jobs.filter((j) => j.assigneeId !== access.crewId); workspace.mileage = workspace.mileage.filter((e) => e.crewId !== access.crewId); await saveWorkspace(access.ownerEmail, workspace); }

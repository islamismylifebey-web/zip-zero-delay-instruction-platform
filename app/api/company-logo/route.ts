...
export async function POST(request: Request) {
  ...
  const saved = await saveWorkspace(access.ownerEmail, workspace, row.updatedAt); if (!saved.ok) return saved.response;
...
export async function DELETE() {
  ...
  if (row) { const workspace = parseWorkspace(row.data); delete workspace.company.logoUpdatedAt; const saved = await saveWorkspace(access.ownerEmail, workspace, row.updatedAt); if (!saved.ok) return saved.response; }

...
export async function POST(request: Request) {
  ...
  if (!employeeId || employeeId.length > 128 || !presetId || presetId.length > 128 || rawInstruction.length < 1 || rawInstruction.length > 2000) return Response.json({ error: 'Choose an employee and preset, then enter a 1–2,000 character instruction.' }, { status: 400 });

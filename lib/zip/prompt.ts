...
export const ZIP_RESPONSE_SCHEMA = {
  type: 'object',
  oneOf: [
    { type: 'object', properties: { status: { const: 'ready' }, professionalAssignment: { type: 'string', minLength: 1, maxLength: 4000 }, sourceIds: { type: 'array', minItems: 1, maxItems: 3, items: { type: 'string' } }, checklist: { type: 'array', maxItems: 20, items: { type: 'string' } }, unsupportedClaims: { type: 'array', maxItems: 20, items: { type: 'string' } } }, required: ['status', 'professionalAssignment', 'sourceIds', 'checklist', 'unsupportedClaims'], additionalProperties: false },
    { type: 'object', properties: { status: { const: 'needs_clarification' }, clarifyingQuestion: { type: 'string', minLength: 1, maxLength: 300 }, missingFields: { type: 'array', maxItems: 20, items: { type: 'string' } }, conflictingFields: { type: 'array', maxItems: 20, items: { type: 'string' } } }, required: ['status', 'clarifyingQuestion', 'missingFields', 'conflictingFields'], additionalProperties: false },
  ],
} as const;

...
export type ZipTokenUsage = { inputTokens: number; outputTokens: number; totalTokens: number };
type ResponsePayload = { output?: Array<{ content?: Array<{ type?: string; text?: string }> }>; usage?: { input_tokens?: number; output_tokens?: number; total_tokens?: number } };
...
export function createOpenAIProvider(apiKey: string, fetcher: typeof fetch = fetch, onUsage?: (usage: ZipTokenUsage) => void): ZipProvider {
  ...
  return { async generate(rawInstruction, context, signal) {
    ...
    if (payload.usage && onUsage) onUsage({ inputTokens: Number(payload.usage.input_tokens ?? 0), outputTokens: Number(payload.usage.output_tokens ?? 0), totalTokens: Number(payload.usage.total_tokens ?? 0) });

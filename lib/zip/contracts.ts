export type ZipDraftRequest = { employeeId: string; presetId: string; rawInstruction: string };
export type ZipCompanyContext = { sourceId: string; name: string; policies: string[]; services?: string; workingHours?: string; timeZone?: string; locations?: string[]; escalationContact?: string; provenance?: string };
export type ZipPresetContext = { sourceId: string; id: string; name: string; purpose: string; steps: string[]; safetyRules: string[]; evidence: string[]; completionDefinition: string; tools?: string[]; requiredTraining?: string[] };
export type ZipEmployeeContext = { sourceId: string; id: string; displayName: string; role: string; department: string; authorizedJobTypes: string[]; training: string[]; preferredLanguage: string; communicationDetail: 'brief' | 'standard' | 'detailed'; responsibilities: string[]; tone?: 'friendly' | 'professional' };

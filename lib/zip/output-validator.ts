...
function containsUnsupportedConcreteDetail(text: string, context: VerifiedZipContext) {
  const corpus = normalized(JSON.stringify(context));
...
export function validateZipModelResult(value: unknown, context: VerifiedZipContext): ZipModelResult | InvalidResult {
  ...
  if (value.status === 'ready') {
    ...
    if (containsUnsupportedConcreteDetail(value.professionalAssignment, context)) return { status: 'invalid', reasons: ['unsupported_concrete_detail'] };

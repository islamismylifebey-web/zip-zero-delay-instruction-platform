...
export type ZipContextError = 'employee_not_found' | 'preset_not_found' | 'company_mismatch' | 'job_not_authorized' | 'training_not_confirmed';
...
export function buildVerifiedContext(request: ZipDraftRequest, source: ZipContextSource): ZipContextResult {
  ...
  if (!employee.authorizedJobTypes.includes(preset.id)) return { ok: false, code: 'job_not_authorized' };
  const training = new Set(employee.training.map((value) => value.trim().toLowerCase()));
  if (preset.requiredTraining?.some((value) => !training.has(value.trim().toLowerCase()))) return { ok: false, code: 'training_not_confirmed' };
  return {
    ok: true,
    context: structuredClone({
      company: source.company,
      preset: { sourceId: preset.sourceId, name: preset.name, purpose: preset.purpose, steps: preset.steps, safetyRules: preset.safetyRules, evidence: preset.evidence, completionDefinition: preset.completionDefinition, ...(preset.tools ? { tools: preset.tools } : {}), ...(preset.requiredTraining ? { requiredTraining: preset.requiredTraining } : {}) },
      employee: { sourceId: employee.sourceId, displayName: employee.displayName, role: employee.role, department: employee.department, authorizedJobTypes: employee.authorizedJobTypes, training: employee.training, preferredLanguage: employee.preferredLanguage, communicationDetail: employee.communicationDetail, responsibilities: employee.responsibilities, ...(employee.tone ? { tone: employee.tone } : {}) },
    }),
  };
}

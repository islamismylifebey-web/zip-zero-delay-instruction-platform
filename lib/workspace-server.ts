...
export function normalizeMemberRole(value: unknown): CrewMember['accessLevel'] { return typeof value === 'string' && memberRoles.has(value as CompanyRole) ? value as CrewMember['accessLevel'] : 'employee'; }
export function sanitizeWorkspace(workspace: WorkspaceData): WorkspaceData { return stripRawInstructionFields(workspace); }
export function parseWorkspace(data: string): WorkspaceData {
  ...
  return {
    company: { name: typeof parsed.company?.name === 'string' ? parsed.company.name : '', departments: Array.isArray(parsed.company?.departments) ? parsed.company.departments.filter((item): item is string => typeof item === 'string') : [], defaultRole: normalizeMemberRole(parsed.company?.defaultRole), contactName: typeof parsed.company?.contactName === 'string' ? parsed.company.contactName : '', contactEmail: typeof parsed.company?.contactEmail === 'string' ? normalizeEmail(parsed.company.contactEmail) : '', contactPhone: typeof parsed.company?.contactPhone === 'string' ? parsed.company.contactPhone : '', ...(parsed.company?.logoUpdatedAt ? { logoUpdatedAt: parsed.company.logoUpdatedAt } : {}) },
    jobs: stripRawInstructionFields({ jobs: (Array.isArray(parsed.jobs) ? parsed.jobs : []).filter((job) => !legacyDemoIds.has(job.assigneeId)) }).jobs as Job[],
    crew,

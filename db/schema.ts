...
export const zipPendingDrafts = sqliteTable("zip_pending_drafts", {
  ...
  knowledgeVersion: text("knowledge_version").notNull().default(""),
  presetId: text("preset_id").notNull().default(""),
}, (table) => [index("zip_pending_drafts_owner_email_idx").on(table.ownerEmail), index("zip_pending_drafts_expires_at_idx").on(table.expiresAt)]);

export const zipCompanyKnowledge = sqliteTable("zip_company_knowledge", {
  ownerEmail: text("owner_email").primaryKey().references(() => workspaceStates.ownerEmail, { onDelete: "cascade" }),
  data: text("data").notNull(),
